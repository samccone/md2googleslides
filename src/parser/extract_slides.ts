  ...
  if (fragment.childNodes?.length) {
    ...
    switch (node.nodeName) {
      ...
      case '#comment':
        ...
        return;
      default:
        console.warn('Unsupported inline HTML element: ' + node.nodeName);
    }
...
fullTokenRules['html_block'] = (token, context) => {
  ...
  if (match === null) {
    console.warn('Unsupported HTML block: ' + token.content);
    // Just assume the text is intentional and add it
    context.appendText(token.content);
    return;
  }
